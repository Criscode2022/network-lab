...
@Component({
  ...
  template: `
    <button
      ...
      [class.ring-1]="onSyllabus()"
      [class.ring-brand-500/40]="onSyllabus()"
      aria-haspopup="listbox"
      [attr.aria-expanded]="open()"
      [attr.aria-label]="t('lab.labs')"
      (click)="open.set(!open())"
    >
      <span class="flex min-w-0 items-center gap-2">
        @if (currentIndex() > 0) {
          ...
          <span class="chip chip-muted shrink-0 px-1.5">{{ t('lab.labs') }}</span>
          <span class="truncate text-ink-300">{{ t('lab.choose') }}</span>
        }
      </span>
    ...
    @if (open()) {
      <div class="card absolute left-0 z-40 mt-1.5 w-[min(92vw,28rem)] overflow-hidden animate-pop" role="listbox" [attr.aria-label]="t('lab.labs')">
