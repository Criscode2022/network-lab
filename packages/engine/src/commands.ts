...
export const COMMANDS: Record<DeviceKind, CmdHelp[]> = {
  workstation: [
    { cmd: 'ip addr [add|del ADDR/P dev IF]', help: 'Show, add or remove an IPv4/IPv6 address' },
    { cmd: 'ip link set IF up|down', help: 'Admin state' },
    { cmd: 'ip route [add|del|replace default via GW]', help: 'Show, add, remove or change a route (add refuses a second default)' },
  ...
  switch: [
    ...
    { cmd: 'ip address A.B.C.D MASK | no ip address', help: 'SVI IPv4 (management, no inter-VLAN routing)' },
    { cmd: 'ipv6 address CIDR', help: 'SVI IPv6' },
    { cmd: 'ip default-gateway A.B.C.D | no ip default-gateway', help: 'Management default gateway' },
  ...
  router: [
    ...
    { cmd: 'ip address / no ip address / ipv6 address / no shutdown', help: 'Addressing' },
    { cmd: 'ipv6 nd prefix CIDR', help: 'RA prefix (SLAAC)' },
    { cmd: 'ipv6 nd suppress-ra / no ipv6 nd suppress-ra', help: 'RA enable' },
    { cmd: 'ip route NET MASK NH / no ip route NET MASK [NH] / ipv6 route', help: 'Static routes (0.0.0.0 0.0.0.0 = default)' },
  ...
  ap: [
    ...
    { cmd: 'interface wlan0 | Gi0/1, then shutdown / no shutdown', help: 'Radio or uplink admin state (bare "no shutdown" enables both)' },
